...
> **Интеллектуальная система работы с документами на базе RAG (Retrieval-Augmented Generation)**
...
� **Promo Page**: [pierrot.merkurov.love/ai](https://pierrot.merkurov.love/ai)
...
- **Векторный поиск** по семантике, а не по ключевым словам
- **Гибридный поиск** (30% ключевые слова + 70% семантика) для максимальной точности
- **HNSW индекс** для быстрого поиска по 1536-мерным векторам OpenAI
- Поддержка русского и английского языков

### 💬 Интеллектуальный чат

- **GPT-4o-mini** для генерации ответов с низкой задержкой
- **Определение намерений**: автоматически распознаёт тип запроса (анализ, сравнение, Q&A)
- **Цитирование источников** с указанием релевантности и прямыми ссылками
- **История разговоров** с возможностью продолжить диалог
- **Markdown-рендеринг** для красивого форматирования ответов

### 📚 Работа с документами

- Поддержка форматов: `.txt`, `.docx` (через библиотеку mammoth)
- Умное разбиение на чанки (до 2000 символов с перекрытием 200)
- Автоматическая очистка от служебных символов и null bytes
...
- **Психолингвистический анализ**: языковые паттерны, эмоциональный тон, риторические приёмы
...
- **Сравнение документов**: выявление различий в стиле и содержании

### 💰 Бюджет-оптимизация

- **$0.78/месяц** при активном использовании (одним пользователем)
- Условный reranking через LLM (только при similarity < 0.5)
- Минимизированная телеметрия
- Эффективное кэширование и батчинг запросов
...
- **Tailwind CSS** + **@tailwindcss/typography** для стилизации
...
- **Vercel AI SDK** (@ai-sdk/openai) для работы с embeddings
- **Next.js API Routes** для серверной логики

### База данных

- **Supabase PostgreSQL** с расширением **pgvector**
- **HNSW индекс** для векторного поиска (оптимизирован под 32MB maintenance_work_mem)
- **tsvector** для full-text search на русском языке
- Хранимые процедуры: `match_documents`, `hybrid_search`

### AI/ML

- **OpenAI GPT-4o-mini** ($0.60/месяц для генерации)
- **OpenAI text-embedding-3-small** (1536 размерностей, $0.03 одноразово)
- **Vercel AI SDK** для streamline интеграции
...
│                     Client (Browser)                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ ChatAssistant│  │ FileUploader │  │   Sidebar    │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└────────────────────────────┬────────────────────────────────┘
                             │
                             ▼
...
│  │              │  │              │  │              │      │
│  │ • Intent     │  │ • Mammoth    │  │ • History    │      │
│  │ • Vector     │  │ • Chunking   │  │ • Messages   │      │
│  │ • LLM        │  │ • Embeddings │  │              │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└────────────────────────────┬────────────────────────────────┘
                             │
...
│  │ conversations (title, created_at, updated_at)        │   │
...
│  │ • GIN (content_tsv) for full-text search             │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                             │
                             ▼
...
SUPABASE_SERVICE_ROLE_KEY=<REDACTED - set in environment, do NOT commit>
...
1. **COMPLETE_FIX.sql** - создание основных таблиц
2. **FIX_MESSAGES_TABLE.sql** - таблица для истории чатов
3. **REINDEX_VECTORS.sql** - векторные индексы
